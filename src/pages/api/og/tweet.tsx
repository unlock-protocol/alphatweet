...
async function TweetOgHandler(request: NextRequest) {
  ...
  const author = searchParams.get("author") ?? "unlock-protocol.eth";
  return new ImageResponse(
    (
      <div tw="flex flex-col w-full h-full items-center relative rounded justify-center bg-white bg-black">
        ...
        <p tw="text-white text-5xl absolute top-24 left-24 max-w-[900px] font-bold">
          {title}
        </p>
        <div tw="flex flex-col bottom-24 left-24 absolute">
          <div tw="text-white text-3xl font-semibold mb-4"> Created by </div>
          <div tw="bg-white px-4 py-3 text-3xl font-bold rounded-lg ">
            {author}
          </div>
        </div>
      </div>
